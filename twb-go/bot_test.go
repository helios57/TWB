package main

import (
	"fmt"
	"net/http"
	"net/http/httptest"
	"testing"
	"twb-go/game"
)

func TestBot(t *testing.T) {
	// Create a mock server
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		html := `
			<script>
				TribalWars.updateGameData({
					"village": {
						"id": 123,
						"name": "Test Village",
						"wood": 5000.5,
						"stone": 5000.2,
						"iron": 5000.8,
						"pop": 100,
						"pop_max": 1000,
						"storage_max": 2000,
						"buildings": {
							"main": "1",
							"barracks": "1"
						}
					}
				});
			</script>
			<table id="units_home">
				<tr><th>Unit</th><th>Count</th></tr>
				<tr>
					<td class="unit-item unit-item-spear">100</td>
					<td class="unit-item unit-item-sword">50</td>
					<td class="unit-item unit-item-axe">25</td>
				</tr>
			</table>
		`
		fmt.Fprintln(w, html)
	}))
	defer server.Close()

	// Create a new Bot
	bot, err := NewBot()
	if err != nil {
		t.Fatalf("NewBot failed: %v", err)
	}
	bot.Wrapper.Endpoint = server.URL
	bot.Villages[0].BuildingManager.SetQueue([]string{"main:2"})
	bot.Villages[0].TroopManager.SetWanted(map[string]map[string]int{"barracks": {"spear": 150}})
	bot.Villages[0].BuildingManager.Costs["main"] = game.BuildingCost{Wood: 100, Stone: 100, Iron: 100, Pop: 10, CanBuild: true}
	bot.Villages[0].TroopManager.RecruitData["spear"] = game.UnitCost{Wood: 50, Stone: 30, Iron: 10, Pop: 1, RequirementsMet: true}

	// Run the bot
	for _, v := range bot.Villages {
		v.Run()
	}

	// Assert that the managers' states are updated correctly
	village := bot.Villages[0]
	if village.ResourceManager.Actual.Wood != 2400 {
		t.Errorf("Expected wood to be 2400, got %d", village.ResourceManager.Actual.Wood)
	}
	if village.BuildingManager.Levels["main"] != 2 {
		t.Errorf("Expected main level to be 2, got %d", village.BuildingManager.Levels["main"])
	}
	if village.TroopManager.TotalTroops["spear"] != 150 {
		t.Errorf("Expected spear to be 150, got %d", village.TroopManager.TotalTroops["spear"])
	}
}
